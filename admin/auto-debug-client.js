...
(function() {
  ...
  function monitorComponentRender() {
    // 监听 React.createElement 调用
    // 注意：此计数统计的是 createElement 调用次数（即 JSX 元素创建数量），
    // 而非组件重渲染次数。每个 <div>、<span>、<Icon /> 等 JSX 标签
    // 都会触发一次 createElement，因此数值通常在 100-500 之间属于正常范围。
    ...
    if (originalCreateElement) {
      ...
      setTimeout(function() {
        console.log(`%c[Auto Debug] React 元素创建数: ${elementCount}（含所有 JSX 标签及子元素，非组件重渲染次数）`, 'color: #1890ff;');
        
        if (elementCount === 0) {
          console.warn('%c[Auto Debug] React 未创建任何元素，可能是组件加载失败', 'color: #faad14;');
        }
      }, 2000);
    }
  }
